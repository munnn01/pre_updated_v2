...
bootstrap resamples. This is the only retained results package: the aggregate
JSONs retain the winning frozen V2-C policy, shard IDs, fingerprints, full
curves, and concurrent control measurements for auditability.

| Codec | Analyzer | V2-C Top-1 BD-rate | 95% bootstrap CI | BD-accuracy (pp) |
|---|---|---:|---:|---:|
...
some `r2plus1d_18` gain against the older single-analyzer control for much
stronger `r3d_18` transfer. Neither analyzer is independent of V2 policy
development. Moreover, the same TEST set was inspected in earlier V1 work:
